...
public:
    ...
    //my functions
    //returns head
    shared_ptr<pq> GetHead();
    //prints text sequentially in Linked List
    void print();
    //calls InitPriorityQueue(), and sets obj->text to s and ->priority to p 
    shared_ptr<pq> set(string s, float p);
private:
    // you can add add more private member variables and member functions here if you need
    shared_ptr<pq> head=NULL;
};
