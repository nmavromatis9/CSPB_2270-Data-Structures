...
//my functions
  ...
  void RemoveHelp(shared_ptr<bst_node>& node, int data);
  //This finds and returns the parent of node (which contains data)
  shared_ptr<bst_node> GetParent(shared_ptr<bst_node> node, int data);
  //Find and return InOrder successor of Node
  //Which is leftmost node of right subtree of Node
  shared_ptr<bst_node> InOrder(shared_ptr<bst_node> node);
//
