...
public:
  ...
  shared_ptr<btree> build_semi_full_two_tier();

  shared_ptr<btree> build_thin_three_tier();
  
  shared_ptr<btree> build_two_tier_spaced();

  bool almostFull(shared_ptr<btree> node);

  bool isFull(shared_ptr<btree> node);
  ...
  //returns index of correct child node containing key (of parent)
  int findChild(shared_ptr<btree> parent, int key);
  shared_ptr<btree> findParent(shared_ptr<btree> root, int key);
